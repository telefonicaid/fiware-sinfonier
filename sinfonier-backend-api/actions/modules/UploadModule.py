...
from error.ErrorHandler import Error, ModuleVersionInvalidId, ModuleException, ModuleErrorCompilingException, \
    MissingMandatoryFields, ModuleInvalidId, GistHandlerUrlException, GistHandlerIdInvalid, ModuleInvalidLanguage
...
class UploadModule(ApiBase):
    ...
    def on_post(self, req, res, id, version_id):
        ...
        except GistHandlerUrlException as Ex:
            logger.error(Ex.message)
            return ResponsesHandler.handle_805(res, Ex.message)
        except GistHandlerIdInvalid as Ex:
            logger.error(Ex.message)
            return ResponsesHandler.handle_805(res, Ex.message)
