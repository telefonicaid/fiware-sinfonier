(function () {
  ...
  lang.extend(WireIt.WiringEditor, WireIt.BaseEditor, {
    ...
    save: function () {
      ...
      this.tempSavedWiring = {prev: previous, name: value.name, working: value.working, language: this.options.languageName};
    ...
    loadPipe: function (name) {
      ...
      try {
        ...
        document.getElementById("previous").value = wiring.properties.name;
        document.getElementById("templateid").value = wiring.properties.templateid || "";
    ...
    getValue: function () {
      ...
      for (i = 0; i < this.layer.containers.length; i++) {
        var values = this.layer.containers[i].getValue();
        obj.modules.push({
          name: this.layer.containers[i].name,
          type: this.layer.containers[i].type,
          language: this.layer.containers[i].language,
          versionCode: this.layer.containers[i].versionCode,
          module_id: this.layer.containers[i].module_id,
          module_version_id: this.layer.containers[i].module_version_id,
          value: values.values,
      ...
      var wiring = this.pipesByName[obj.properties.name] || {};

      return {
        name: obj.properties.name,
        description: obj.properties.description,
        sharing: wiring.sharing,
        working: obj
      };
    }
