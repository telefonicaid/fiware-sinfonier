...
public class Modules extends BaseController {
  ...
  public static void save(@Valid Module module, @Valid ModuleVersion version) throws SinfonierException {
    ...
    if (validation.hasErrors()) {
      ...
      if (module.getVersions().getVersions().size() > 1) {
        ...
        for (Version tmpVersion : versions) {
          if (version.getId() == null || !tmpVersion.getModuleVersion().getId().equals(version.getId())) {
        ...
        if (moduleVersion != null) {
          ...
          if (versions < LIMIT_PENDING_MODULE_VERSIONS) {
            ...
            if (res.get("code") != null && res.get("code").getAsInt() == Client.STATUS_SUCCESS) {
              ...
              if (res.get("data") != null && res.get("data").getAsJsonObject().get("trace") != null) {
                flash.put(FLASH_KEY_BACKEND_TRACE, res.get("data").getAsJsonObject().get("trace").getAsJsonObject().get("message").getAsString());
              }

              Logger.error(res.toString());
            } else {
              flash.put(FLASH_KEY_BACKEND_LOG, Messages.get("Modules.recheck.error"));
              Logger.error(res.toString());
            }
  ...
  public static void check() throws SinfonierException {
    try {
      ...
      TopologyModule.checkTopologyModule(jTopologyModule);
        ...
        c400.setMessageData(se.getError() != null ? se.getError().getMessagei18n() : se.getMessage());
